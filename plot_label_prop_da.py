...
XX, YY = np.meshgrid(
    np.linspace(ax[0], ax[1], 100), np.linspace(ax[2], ax[3], 100)
)
...
clf = make_da_pipeline(
    OTLabelPropAdapter(), KernelRidge(kernel="rbf", alpha=0.5)
)
...
plt.scatter(Xs[:, 0], Xs[:, 1], c=ys, label="Prediction")
plt.imshow(
    Zjdot, extent=(ax[0], ax[1], ax[2], ax[3]), origin="lower", alpha=0.5
)
